...
Users on versions before 0.1.80 need one manual replacement. Later versions download updates in the background and show **Restart to update**. Restart preparation blocks active/queued tasks, extra app windows, and terminal work. A failed preparation always leaves update mode, restoring any tab it can. The Chromium updater saves chat drafts, workspace state, and browser tab addresses/order/selection before automatically closing pages for restart. Restored browser tabs load their saved addresses; this does not serialize a website's in-memory state, form contents, or navigation history. Pages with possible unsaved edits or other unsafe activity keep the update ready until that work is saved or finished, and the message names the tab and what to do. Only pages the user (or an agent) has clicked, typed in, or dropped something onto since they loaded are checked for edits: an untouched page holds no draft of the user's, so its fields, frames and editors don't block. Aven records that input itself, because Chromium's activation flag also counts pages opened from the address bar. Back, forward and reload keep a page marked, since they can restore form values. On a touched page the checks are conservative: filled forms, embedded frames, and complex editors can require attention even when the website has already saved its work. Loading pages, downloads, playing media, camera or microphone use, and open dialogs block on any page. A persistence failure must leave the app running without installing the update. The WebKit fallback still requires manually closing browser pages.
