...
import { loadDefaultRuntimeMode, type RuntimeMode } from "./runtimeMode";
export {
...
export type Block = {
  /** Provider item identity while streaming; distinct adjacent items stay separate. */
  messageKey?: string;
  id: string;
  role: BlockRole;
  text: string;
  ...
  startedAt?: number;
  /** Model captured when this user turn starts; picker changes target later turns. */
  turnModel?: { harness: HarnessId; model: string };
  ...
  /** App-authored continuation: sent to the harness but hidden from the user transcript. */
  internal?: boolean;
  handoff?: HandoffMeta;
...
export type Session = {
  ...
  busy?: boolean;
  /** Follow-ups waiting for the current turn; restored paused from durable history. */
...
/** A blank split is a new task; only its folder comes from the adjacent task. */
export function newSplitSession(
  source: Pick<Session, "cwd"> | undefined,
  fallbackCwd = "~",
): Session {
  return newDefaultSession(source?.cwd ?? fallbackCwd);
}
