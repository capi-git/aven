...
export type SessionFolder = {
  ...
  sessionIds: string[];
  collapsed: boolean;
  /** Set by a member drag; otherwise members keep pin/recency sorting. */
  manualOrder?: boolean;
...
export type SessionListDropTarget =
  ...
  | { kind: "session"; id: string; edge?: "before" | "after" };
...
export function buildSessionList(
  ...
  for (const folder of folders) {
    ...
    if (!folder.manualOrder) members.sort(compareSessionSummaries);
 ...
 * An insertion edge on a sibling sets a persistent order within its folder.
 */
export function applySessionListDrop(
  ...
  if (dest) {
    if (folderContaining(folders, draggedId)?.id === dest.id) {
      if (!target.edge) return { folders };
      // Start the first manual move from the order the user sees, including
      // hidden members, rather than the folder's original insertion order.
      const currentIds = dest.manualOrder
        ? dest.sessionIds
        : orderByIds(
            dest.sessionIds.map((id) => ({ id })),
            [...sessions]
              .sort(compareSessionSummaries)
              .map((session) => session.id),
          ).map(({ id }) => id);
      const sessionIds = currentIds.filter((id) => id !== draggedId);
      const index = sessionIds.indexOf(target.id);
      if (index < 0) return { folders };
      sessionIds.splice(
        index + (target.edge === "after" ? 1 : 0),
        0,
        draggedId,
      );
      if (sessionIds.every((id, i) => id === currentIds[i])) return { folders };
      return {
        folders: folders.map((folder) =>
          folder.id === dest.id
            ? { ...folder, sessionIds, manualOrder: true }
            : folder,
        ),
      };
    }
...
function parseFolder(value: unknown): SessionFolder | null {
  ...
  return {
    ...
    ...(rec.manualOrder === true ? { manualOrder: true } : {}),
    ...(customColor != null
      ? { customColor }
      : colorIndex != null
        ? { colorIndex }
