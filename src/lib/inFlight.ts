...
const LEGACY_INTERRUPT_MESSAGES = new Set([
  "Turn interrupted when CoveCode quit.",
...
export function quitWhileBusyMessage(count: number): string {
  if (count === 1) {
    return "1 chat is still running. Quit anyway? It will resume when you reopen Aven.";
  }
  return `${count} chats are still running. Quit anyway? They will resume when you reopen Aven.`;
}
...
function lastBlockIsInterrupt(session: Session): boolean {
  ...
  return (
    last?.role === "system" &&
    (last.text === INTERRUPT_MESSAGE ||
      LEGACY_INTERRUPT_MESSAGES.has(last.text))
  );
}
