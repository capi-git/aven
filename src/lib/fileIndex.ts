...
export function subscribeProjectFiles(listener: Listener): () => void {
  ...
  if (listeners.size === 1 && !cache && !inflight) scheduleIndexRefresh();
  return () => {
...
function scheduleIndexRefresh() {
  if (!lastCwd || listeners.size === 0) return;
...
function onDirectoriesChanged() {
  // Agent writes invalidate the index; only an open file consumer warrants a
  // fresh recursive scan. On-demand resolution still calls loadProjectFiles.
  if (lastCwd) invalidateProjectFiles(lastCwd);
  scheduleIndexRefresh();
}

subscribeDirsChanged(onDirectoriesChanged, { refreshCache: false });

if (typeof document !== "undefined") {
  window.addEventListener("focus", () => {
    if (!document.hidden) onDirectoriesChanged();
  });
