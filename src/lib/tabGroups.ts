...
const TAB_GROUP_LABELS_CHANGED = "monocode:tab-group-labels-changed";
...
export function saveTabGroupLabel(project: string, label: string): void {
  ...
  if (writeRecord(LABEL_KEY, next) && typeof window !== "undefined")
    window.dispatchEvent(new Event(TAB_GROUP_LABELS_CHANGED));
}

export function subscribeTabGroupLabels(listener: () => void): () => void {
  const onStorage = (event: StorageEvent) => {
    if (event.key === LABEL_KEY || event.key === null) listener();
  };
  window.addEventListener(TAB_GROUP_LABELS_CHANGED, listener);
  window.addEventListener("storage", onStorage);
  return () => {
    window.removeEventListener(TAB_GROUP_LABELS_CHANGED, listener);
...
function withGroup<T extends GroupedTab>(
  tab: T,
...
export function ungroupTabs<T extends GroupedTab>(
  tabs: T[],
  groupId: string,
): T[] {
  return tabs.map((tab) =>
