...
export function orchestrationPlanningPrompt(
  ...
  return [
    "Prepare an orchestration proposal for the user to review in Aven. Investigate and plan only: do not edit files, start workers, or invoke the Aven control CLI. No execution is authorized until the user confirms the assignment card.",
...
function proposalCandidates(responses: readonly string[]): string[] {
  ...
  return [
    ...new Set([...tagged, ...fenced, ...responses].map((text) => text.trim())),
...
export function completeOrchestrationProposal(
  ...
  try {
    ...
    const responses = typeof response === "string" ? [response] : response;
    const failure = responses
      .flatMap((text) => text.split("\n"))
      .find(isProviderFailureText);
    if (failure) throw new Error(failure.trim());
    let validationError: unknown;
    for (const raw of proposalCandidates(responses)) {
      let parsed: unknown;
      try {
    ...
    throw validationError ?? new Error(INCOMPLETE_PROPOSAL);
  } catch (reason) {
    return {
      ...draft,
      tasks: [],
...
export function restoreOrchestrationProposal(
  ...
  if (
    ...
    /^Could not prepare the assignment card: (?:JSON Parse error|Unexpected (?:token|identifier|end))/i.test(
      value.error ?? "",
    )
  )
    return { ...value, tasks: [], error: INCOMPLETE_PROPOSAL };
