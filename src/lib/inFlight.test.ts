...
describe("canAutoContinue", () => {
  it.each(["CoveCode", "MonoCode"])(
    "preserves continuation and idempotency for interrupted %s sessions",
    (name) => {
      const legacy = chat("/tmp/a", {
        ...
        blocks: [
          { id: "u1", role: "user", text: "hello" },
          {
            id: "old-interrupt",
            role: "system",
            text: `Turn interrupted when ${name} quit.`,
          },
        ],
      });
      expect(canAutoContinue(legacy)).toBe(true);
      expect(markTurnInterrupted(legacy).blocks).toHaveLength(2);
    },
  );
