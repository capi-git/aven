...
export type AgentModel = {
  ...
  name: string;
  nativeId?: string;
  /** Equivalent catalog IDs reported by a provider, used only for picker preferences. */
  pickerAliases?: string[];
  /** Stable resolved identity for preferences; never changes the model sent to the provider. */
  pickerPreferenceId?: string;
...
const HIDDEN_PICKER_MODELS_KEY = "monocode.hiddenPickerModels";
...
export function saveFavoriteModels(ids: string[]) {
  try {
    localStorage.setItem(
      FAVORITES_KEY,
      JSON.stringify([...new Set(ids.map(pickerPreferenceId))]),
    );
  } catch {
    // private mode / quota
  }
...
function onPickerStorage(event: StorageEvent) {
  if (
    event.key == null ||
    event.key === HIDDEN_PICKER_PROVIDERS_KEY ||
    event.key === HIDDEN_PICKER_MODELS_KEY
  )
...
export function subscribePickerVisibility(
  ...
  if (pickerVisibilityListeners.size === 0 && typeof window !== "undefined") {
    window.addEventListener("storage", onPickerStorage);
  }
  pickerVisibilityListeners.add(onStoreChange);
  return () => {
    pickerVisibilityListeners.delete(onStoreChange);
    if (pickerVisibilityListeners.size === 0 && typeof window !== "undefined") {
      window.removeEventListener("storage", onPickerStorage);
    }
  };
}
...
/** Individual choices remain remembered when their provider is switched off. */
export function loadHiddenPickerModels(): string[] {
  try {
    const parsed: unknown = JSON.parse(
      localStorage.getItem(HIDDEN_PICKER_MODELS_KEY) ?? "[]",
    );
...
function pickerIdentities(model: AgentModel): string[] {
  return [
    model.id,
    ...(model.pickerPreferenceId ? [model.pickerPreferenceId] : []),
    ...(model.pickerAliases ?? []),
  ];
}

function pickerModelHidden(model: AgentModel, hidden: Set<string>): boolean {
  return pickerIdentities(model).some((id) => hidden.has(id));
}

/** Resolve a saved picker choice through provider-reported aliases, without changing launch IDs. */
export function findPickerModel(id: string): AgentModel | undefined {
  return (
    findModel(id) ??
    allModels().find((model) => pickerIdentities(model).includes(id))
  );
}

function pickerPreferenceId(id: string): string {
  return findPickerModel(id)?.pickerPreferenceId ?? id;
}

/** The catalog itself stays intact so existing sessions keep their model. */
export function pickerModelsFor(harness: HarnessId): AgentModel[] {
  const hidden = new Set(loadHiddenPickerModels());
  return modelsFor(harness).filter(
    (model) => !pickerModelHidden(model, hidden),
  );
}

export function isPickerModelVisible(model: AgentModel): boolean {
  return (
    isPickerProviderVisible(model.harness) &&
    !pickerModelHidden(model, new Set(loadHiddenPickerModels()))
  );
}

/** Keep at least one catalog model available in each provider. */
export function savePickerModelVisible(id: string, visible: boolean): boolean {
  const hidden = new Set(loadHiddenPickerModels());
  const model = findPickerModel(id);
  const identities = new Set(model ? [id, ...pickerIdentities(model)] : [id]);
  if (visible) {
    for (const equivalent of identities) hidden.delete(equivalent);
  } else {
    if (
      model &&
      pickerModelsFor(model.harness).every((item) =>
        pickerIdentities(item).some((equivalent) => identities.has(equivalent)),
      )
    )
      return false;
    // Persist one resolved identity so the fallback catalog can both hide and
    // restore this choice before live discovery, without leaving a stale alias.
    for (const equivalent of identities) hidden.delete(equivalent);
    hidden.add(pickerPreferenceId(id));
  }
  try {
    localStorage.setItem(HIDDEN_PICKER_MODELS_KEY, JSON.stringify([...hidden]));
  } catch {
    return false;
  }
  emitPickerVisibility();
...
export function showAllPickerModels(harness: HarnessId) {
  const prefix = `${harness}:`;
  const hidden = loadHiddenPickerModels().filter(
...
export function saveDefaultModel(harness: HarnessId, model: string) {
  const next = {
    ...loadDefaultModels(),
    [harness]: pickerPreferenceId(model),
  };
  try {
...
/** User-picked visible model for a provider, else a visible catalog default. */
export function preferredModelId(harness: HarnessId): string {
  const hidden = new Set(loadHiddenPickerModels());
  const isHidden = (id: string) => {
    if (hidden.has(id)) return true;
    // Session resolution already falls back when an old alias is absent; do
    // not let that fallback silently select a model hidden by the user.
    const model = findPickerModel(id) ?? resolveModel(harness, id);
    return model.harness === harness && pickerModelHidden(model, hidden);
  };
  const saved = loadDefaultModels()[harness];
  // A live catalog may not have loaded yet; retain a saved, non-hidden id.
  if (saved && !isHidden(saved)) return findPickerModel(saved)?.id ?? saved;
  const last = loadLastModelChoice();
  if (last?.harness === harness && !isHidden(last.model))
    return findPickerModel(last.model)?.id ?? last.model;
  const fallback = defaultModelId(harness);
  if (!isHidden(fallback)) return fallback;
  return pickerModelsFor(harness)[0]?.id ?? "";
}

/** Provider + model new conversations should start with; hidden choices never win. */
export function defaultSessionChoice(): LastModelChoice {
  const last = loadLastModelChoice();
  const preferred = last?.harness ?? "cursor";
  const usable = (id: HarnessId) =>
    isPickerProviderVisible(id) &&
    (modelsFor(id).length === 0 || pickerModelsFor(id).length > 0);
  const harness = usable(preferred)
    ? preferred
    : (HARNESS_ORDER.find(usable) ?? preferred);
