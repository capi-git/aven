...
export function hydrateWorkspaceSnapshot(
  ...
  const take = (id: string): Session | null => {
    ...
    if (stub?.draft) restoreComposerDraft(id, stub.draft);
    const restored =
      stub?.queuedMessages !== undefined
        ? { ...base, ...restoredMessageQueue(stub.queuedMessages) }
        : base;
...
function sessionStub(session: Session): WorkspaceSessionStub | null {
  ...
  return {
    ...
    title: session.title,
    queuedMessages: persistedMessageQueue(session.queuedMessages),
    ...
    ...(draft ? { draft: sanitizeComposerDraft(draft) } : {}),
  };
}
...
function sessionFromStub(stub: WorkspaceSessionStub): Session {
  ...
  return {
    ...session,
    ...restoredMessageQueue(stub.queuedMessages),
    id: stub.id,
    title: stub.title,
...
function sanitizeStub(raw: unknown): WorkspaceSessionStub | null {
  ...
  const modelSettings =
    ...
    !Array.isArray(value.modelSettings)
      ...
      : {};
  const draft = sanitizeComposerDraft(value.draft);
  return {
    id: value.id,
    cwd:
    ...
    ...(Array.isArray(value.queuedMessages)
      ? { queuedMessages: persistedMessageQueue(value.queuedMessages) }
      : {}),
  };
}
