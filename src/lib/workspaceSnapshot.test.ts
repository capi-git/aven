...
describe("collectWorkspaceSnapshot", () => {
  it("saves unsent standalone drafts separately from transcripts, including pasted attachments", () => {
    const session = newSession(
      "cursor",
      "/app-data/projectless-workspaces/work",
    );
      ...
      attachments: [
        {
          ...
          size: 4,
          data: "cGl4ZWw=",
          previewUrl: "blob:expired-after-restart",
        },
      ],
    });
    ...
    expect(snapshot.sessions[0]?.draft).toMatchObject({
      text: "Standalone Work draft — local QA, do not send.",
      attachments: [{ data: "cGl4ZWw=" }],
    });
    expect(
      snapshot.sessions[0]?.draft?.attachments[0].previewUrl,
    ).toBeUndefined();
...
describe("hydrateWorkspaceSnapshot", () => {
  it("restores two blank standalone tabs and their independent drafts from a serialized workspace", () => {
    const personal = newSession(
      "cursor",
      "/app-data/projectless-workspaces/personal",
    );
    const work = newSession("cursor", "/app-data/projectless-workspaces/work");
    const tabs = [newTab(personal.id), newTab(work.id)];
    const snapshot = collectWorkspaceSnapshot(
      tabs,
      [personal, work],
      tabs[1].id,
    ...
    snapshot.sessions[0].draft = {
      text: "Personal draft",
      attachments: [],
      updatedAt: 1,
    };
    snapshot.sessions[1].draft = {
      text: "Standalone Work draft — local QA, do not send.",
      attachments: [],
      updatedAt: 2,
    };
    ...
    expect(workspace?.activeTabId).toBe(tabs[1].id);
    expect(workspace?.projectCwd).toBe(work.cwd);
    expect(
      workspace?.sessions.every((session) => session.blocks.length === 0),
    ).toBe(true);
    expect(readComposerDraft(personal.id)?.text).toBe("Personal draft");
    expect(readComposerDraft(work.id)?.text).toBe(
      "Standalone Work draft — local QA, do not send.",
    );
  });

  it("restores a next-turn draft when the session transcript comes from the session store", () => {
    const session = chat(crypto.randomUUID(), "/tmp/real-project");
    const tab = newTab(session.id);
    const snapshot = collectWorkspaceSnapshot(
      [tab],
      [session],
    ...
    expect(workspace?.sessions[0].blocks).toEqual(session.blocks);
    expect(readComposerDraft(session.id)?.text).toBe("An unsent follow-up");
  });
...
it("reopens accepted queue items paused with their cards and images, even without a database record", () => {
  const session = chat("queue-reopen", "/tmp/project");
  ...
  session.queuedMessages = [
    {
      ...
      intent: "plan",
      noteCard: { id: "n", slug: "n", title: "Note", body: "instructions" },
    },
    {
      id: "q2",
      text: "second",
      attachments: [],
      handoffCard: { from: "claude", to: "codex", brief: "Context" },
    },
  ];
  const tab = newTab(session.id);
  const snapshot = collectWorkspaceSnapshot(
  ...
  const restored = hydrateWorkspaceSnapshot(parsed, new Map())!.sessions[0];
  expect(restored.queuedMessages).toEqual(session.queuedMessages);
  expect(restored.queueStatus).toBe("paused");
  expect(restored.busy).toBe(false);
});

it("uses the newest workspace queue, including an empty queue, while keeping legacy database queues", () => {
  const session = chat("queue-cleared", "/tmp/project");
  const tab = newTab(session.id);
  const snapshot = collectWorkspaceSnapshot(
    [tab],
    [session],
  ...
  const oldRecord = {
    ...session,
    queuedMessages: [{ id: "old", text: "already sent", attachments: [] }],
  ...
  delete snapshot.sessions[0].queuedMessages;
  expect(
    hydrateWorkspaceSnapshot(snapshot, new Map([[session.id, oldRecord]]))!
      .sessions[0].queuedMessages,
