...
describe("buildSessionList", () => {
  ...
  it("keeps manually ordered members stable when activity and pin state change", () => {
    const folders = [folder("work", ["a", "b", "c"], { manualOrder: true })];
    const entries = buildSessionList(
      [
        summary("c", { updatedAt: 100, pinned: true }),
        summary("b"),
        summary("a"),
      ],
      folders,
    ...
    if (entries[0]?.kind !== "folder") throw new Error("Expected folder");
    expect(entries[0].sessions.map((session) => session.id)).toEqual([
      "a",
      "b",
      "c",
    ]);
    expect(sessionListNavigationIds(entries, false)).toEqual(["a", "b", "c"]);
  });
...
describe("applySessionListDrop", () => {
  ...
  it("moves a sibling before or after another and preserves folder metadata", () => {
    const other = folder("other", ["z"]);
    const folders = [
      folder("work", ["a", "b", "c"], { customColor: "#abcdef" }),
      other,
    ];
    const before = applySessionListDrop(folders, "c", {
      kind: "session",
      id: "a",
      edge: "before",
    }).folders;
    ...
    expect(before[1]).toBe(other);
    const after = applySessionListDrop(before, "c", {
      kind: "session",
      id: "b",
      edge: "after",
    }).folders;
    expect(after[0]?.sessionIds).toEqual(["a", "b", "c"]);
    expect(folders[0]?.sessionIds).toEqual(["a", "b", "c"]);
  });

  it("starts the first move from displayed pin/recency order, retaining hidden members", () => {
    const folders = [folder("work", ["old", "new", "pinned", "hidden"])];
    const sessions = [
      summary("old", { updatedAt: 1 }),
      summary("new", { updatedAt: 9 }),
      summary("pinned", { pinned: true, updatedAt: 2 }),
      summary("hidden", { archived: true, updatedAt: 5 }),
    ];
    const next = applySessionListDrop(
      folders,
      "old",
      {
        kind: "session",
    ...
    expect(next[0]?.sessionIds).toEqual(["pinned", "old", "new", "hidden"]);
    const visible = buildSessionList(
      sessions.filter((s) => !s.archived),
      next,
      [],
    );
    if (visible[0]?.kind !== "folder") throw new Error("Expected folder");
    expect(visible[0].sessions.map((s) => s.id)).toEqual([
      "pinned",
      "old",
      "new",
    ]);
  ...
  it("does not switch an untouched folder to manual ordering for a no-op drop", () => {
    const folders = [folder("work", ["b", "a"])];
    const sessions = [summary("a", { updatedAt: 2 }), summary("b")];
    expect(
      applySessionListDrop(
        folders,
        "a",
...
describe("session folder persistence", () => {
  ...
  it("restores a dragged member order without recency sorting after reopening", () => {
    const sessions = [
    ...
    saveSessionFolders("/tmp/project", next);
    const restored = loadSessionFolders("/tmp/project");
    expect(restored[0]?.manualOrder).toBe(true);
    const entries = buildSessionList(sessions, restored, []);
    if (entries[0]?.kind !== "folder") throw new Error("Expected folder");
    expect(entries[0].sessions.map((session) => session.id)).toEqual([
      "c",
      "a",
    ...
    const appended = addSessionToFolder(restored, "work", "new");
    expect(appended[0]?.sessionIds).toEqual(["c", "a", "b", "new"]);
    expect(appended[0]?.manualOrder).toBe(true);
    expect(removeSessionFromFolder(appended, "a")[0]?.sessionIds).toEqual([
      "c",
      "b",
      "new",
    ]);
