...
export async function sendOpenCodeTurn(input: SendTurnInput): Promise<void> {
  ...
  live.turns = live.turns
    ...
    .then(async () => {
      ...
      try {
        await syncRuntimeMode(live, input.runtimeMode);
...
async function ensureLive(input: HarnessSessionInput): Promise<Live> {
  ...
  watchChild(
    input.sessionId,
    (line) => {
      const parsed = parseServerUrlFromOutput(line, port);
...
async function syncRuntimeMode(live: Live, mode: RuntimeMode): Promise<void> {
  if (live.runtimeMode === mode) return;
  // Apply access at the next turn boundary, including when tightening it. A
  // failed update must stop the turn rather than run under the previous policy.
  await live.client.updateSession(live.openCodeSessionId, {
    permission: buildOpenCodePermissionRules(mode),
  });
...
async function assertOpenCodeVersion(path: string, cwd: string): Promise<void> {
  ...
  if (!version) {
    throw new Error(
      `Unable to determine OpenCode version. Aven requires v${MINIMUM_OPENCODE_VERSION} or newer.`,
    );
  }
