...
export function parseServerUrlFromOutput(
  output: string,
  expectedPort?: number,
): string | null {
  for (const line of output.split("\n")) {
    ...
    if (match?.[1]) {
      const url = approvedServerUrl(match[1], expectedPort);
      if (url) return url;
    }
    if (trimmed.startsWith(OPENCODE_SERVER_READY_PREFIX)) {
      const fallback = trimmed.match(/(https?:\/\/[^\s]+)/i);
      if (fallback?.[1]) {
        const url = approvedServerUrl(fallback[1], expectedPort);
        if (url) return url;
      }
    }
  }
...
function approvedServerUrl(
  ...
  try {
    ...
    if (
      ...
      !url.port ||
      Number(url.port) === 0 ||
      (expectedPort !== undefined && Number(url.port) !== expectedPort)
    )
      return null;
    return url.origin;
  } catch {
