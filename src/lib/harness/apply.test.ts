...
describe("turn duration", () => {
  it("captures the launched model once and preserves it through picker changes and steering", () => {
    ...
    session = appendSteerUser(
      { ...session, model: "codex:next-model" },
      "also check tabs",
    );
    ...
    expect(session.blocks[1]?.turnModel).toBeUndefined();
    session = appendUser(stopStreaming(session), "next task");
    expect(session.blocks.at(-1)?.turnModel).toEqual({
      harness: "codex",
