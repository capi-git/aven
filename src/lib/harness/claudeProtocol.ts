...
export function runtimeModeToPermission(
  ...
  switch (mode) {
    ...
    default:
      // Override a different default in the user's Claude configuration. The
      // selected app mode must also be the mode the CLI actually launches with.
      return "default";
  }
}
...
export function buildClaudeUserMessage(input: {
  ...
  for (const attachment of input.attachments ?? []) {
    content.push(
      imageContentBlock(attachment) ?? localAttachmentBlock(attachment),
    );
  }
  return {
    type: "user",
...
export function buildClaudeSpawnArgs(input: {
  ...
  // Isolated spawns are Aven's own helper calls (titles, summaries); the
