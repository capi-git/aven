...
import { localAttachmentBlock } from "../attachments";
...
/** A control lead needs loopback access; ordinary sessions keep their sandbox defaults. */
...
export function codexAttachmentInputs(
  files: Attachment[],
  ...
  return files.map((file) => {
    ...
    return localAttachmentBlock(file);
  });
}
...
function mapItemLifecycle(
  ...
  if (itemType === "imageGeneration") {
    ...
    const path = stringField(item, "savedPath");
    const result = stringField(item, "result");
    const source = path
      ? encodeURI(path).replace(/[?#<>]/g, (character) =>
      ...
      : result
        ...
        : null;
    const success =
      stringField(item, "status") === "completed" && !item.failure && !!source;
    const events: HarnessEvent[] = [
      {
        type: "tool.updated",
        callId,
        ...
        ...(!success
          ? { detail: "Image generation did not return an image." }
          : {}),
      },
    ];
    if (success)
      events.push(
        { type: "message.completed" },
        { type: "message.delta", text: `![Generated image](<${source}>)` },
        { type: "message.completed" },
      );
    return { events };
  }
