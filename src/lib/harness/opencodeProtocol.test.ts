...
describe("parseServerUrlFromOutput", () => {
  it("pins server output to the exact loopback port allocated for this process", () => {
    const output = (url: string) => `opencode server listening on ${url}`;
    expect(
      parseServerUrlFromOutput(output("http://127.0.0.1:4096/"), 4096),
    ...
    for (const url of [
      ...
      expect(parseServerUrlFromOutput(output(url), 4096)).toBeNull();
    expect(
      parseServerUrlFromOutput(
        `${output("http://outside.invalid:4096")}\n${output("http://127.0.0.1:4096")}`,
        4096,
      ),
    ).toBe("http://127.0.0.1:4096");
  });
