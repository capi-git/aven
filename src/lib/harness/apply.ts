...
export function applyHarnessEvent(
  ...
  switch (event.type) {
    ...
    case "message.completed":
      return finishRole(session, "assistant", event.key);
...
export function appendUser(
  ...
  return appendBlock(
    ...
    },
    {
      ...
      text,
      startedAt: Date.now(),
      turnModel: { harness: session.harness, model: session.model },
...
/** Provider item keys preserve message boundaries and route late snapshots to their own block. */
function patchStreaming(
  ...
  if (key) {
    while (
      index >= 0 &&
      !(
        session.blocks[index].role === role &&
        session.blocks[index].messageKey === key
      )
    )
      index--;
  }
