...
async function ensureLive(input: HarnessSessionInput): Promise<Live> {
  ...
  if (existing) {
    // Permission/model changes require a new CLI process, not a new
    // conversation. Keep its resume id unless the working directory changes.
...
async function runTurn(live: Live, input: SendTurnInput): Promise<void> {
  ...
  live.streamingMessageId = undefined;
  live.messageText.clear();
...
function handleStreamEvent(live: Live, rec: Record<string, unknown>): void {
  ...
  if (!subagent && event?.type === "message_start") {
    live.streamingMessageId = stringField(asRecord(event.message), "id");
    live.emittedAssistant = "";
  }
...
function handleAssistant(live: Live, rec: Record<string, unknown>): void {
  ...
  if (messageId) {
    live.messageText.set(messageId, already + extra);
    live.completedMessages.add(messageId);
  }
  live.emittedAssistant = "";
  live.streamingMessageId = undefined;
