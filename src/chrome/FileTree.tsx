...
type Props = {
  cwd: string;
  /** Preserve the loaded tree while pausing work for a hidden inspector. */
  enabled?: boolean;
...
export const FileTree = memo(function FileTree({
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    // Keep the last directory snapshot visible during a fresh read on reveal.
    // Unsubscribing while hidden invalidates the cache, not the retained rows.
    setError(null);
  ...
  return (
    ...
    >
      <div
        ref={rootRef}
        tabIndex={-1}
        className="flex h-full min-h-0 flex-col outline-none focus-visible:ring-1 focus-visible:ring-inset focus-visible:ring-accent/60"
        onKeyDown={onKeyDown}
...
function TreeNode({ entry, depth }: { entry: FsEntry; depth: number }) {
  ...
  useEffect(() => {
    if (!enabled || !entry.isDir || !open) return;
  ...
  }, [enabled, entry.isDir, entry.path, open, epoch]);
