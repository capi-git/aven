...
export function ProjectSearch({
  ...
  const completedSearch = useRef<{ key: string; at: number } | null>(null);
  ...
  useEffect(() => {
    ...
    // Reuse recent results during rapid hover returns, then revalidate on a
    // later reopen so edits made while the inspector was hidden are reflected.
    if (
      completedSearch.current?.key === searchKey &&
      Date.now() - completedSearch.current.at < 1_500
    ) {
      setLoading(false);
      return;
    }
    ...
    const timer = window.setTimeout(() => {
      setLoading(true);
      setError(null);
      void searchProject(options)
        .then((result) => {
          if (cancelled) return;
          completedSearch.current = { key: searchKey, at: Date.now() };
  ...
  }, [caseSensitive, cwd, enabled, exclude, include, query, regex, wholeWord]);
