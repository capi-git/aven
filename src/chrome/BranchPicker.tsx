...
export function BranchPicker({
  ...
  const dismiss = (restore: boolean) => {
    ...
    setBlockedBusy(null);
    if (restore && externalAnchor?.isConnected) externalAnchor.focus();
    if (restore || externalAnchor) onCloseRef.current?.();
  };
  ...
  useEffect(() => {
    ...
    setBlockedError(null);
    setBlockedBusy(null);
    if (externalAnchor) onCloseRef.current?.();
  }, [enabled, externalAnchor]);
  ...
  return (
    <div
      className={
        hideTrigger
          ? "contents"
          : "flex max-w-[45%] shrink-0 items-center gap-2.5"
      }
    >
      <div ref={root} className={hideTrigger ? "contents" : "relative min-w-0"}>
        {!hideTrigger ? (
          <button
            type="button"
            title={title}
