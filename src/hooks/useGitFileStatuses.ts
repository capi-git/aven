...
async function load(entry: Entry) {
  // A pending refresh is UI maintenance: closing the panel or hiding the
  // document must not start another git process after an in-flight read.
  ...
  try {
    ...
    if (entry.listeners.size > 0 && !document.hidden) publish(entry, EMPTY);
  } finally {
