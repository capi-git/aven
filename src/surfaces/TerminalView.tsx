...
function cssHexColor(
  expr: string,
  fallback: string,
  parent: HTMLElement,
): string {
  const color = cssColor(expr, fallback, parent);
  if (/^#[\da-f]{6}$/i.test(color)) return color;
  // Computed hex/HSL tokens use rgb/rgba; color-mix(in srgb, ...) retains
  // normalized srgb channels. Other color spaces must not be read as RGB.
  ...
  const channels = match
    .slice(1, 4)
    .map((value) => Number(value) * (rgb ? 1 : 255));
...
function oscColors(parent: HTMLElement) {
...
export function TerminalView({ id, cwd, active, presented = true, onMetaChange }: Props) {
  ...
  useEffect(() => {
    ...
    const captureScreen = () => {
      ...
      const lines: string[] = [];
      for (let i = Math.max(0, buffer.length - 2000); i < buffer.length; i++)
    ...
    const oscFg = term.parser.registerOscHandler(10, (data) =>
      isOscColorQuery(data) ? replyOsc(10, oscColors(host).fg) : false,
    );
    const oscBg = term.parser.registerOscHandler(11, (data) =>
      isOscColorQuery(data) ? replyOsc(11, oscColors(host).bg) : false,
    );
    const oscCursor = term.parser.registerOscHandler(12, (data) =>
      isOscColorQuery(data) ? replyOsc(12, oscColors(host).cursor) : false,
    );
