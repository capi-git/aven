...
function PaneTreeComponent({
  ...
  useEffect(() => {
    if (!visible) cancelPaneDrag.current?.();
  }, [visible]);
    ...
    (fromId: string, event: ReactPointerEvent<HTMLElement>) => {
      ...
      function finish(commit: boolean) {
        ...
        if (
          over &&
          over.id !== fromId &&
          layoutLeaves(layoutRef.current).some((leaf) => leaf.id === over.id)
        ) {
    ...
    <div ref={treeRef} className="relative h-full min-h-0 min-w-0">
      {leaves.map((leaf) => {
        ...
        const backgroundStyle = {
          ...
          "--chat-background-height": `var(--workspace-background-height, ${100 / leaf.rect.h}%)`,
        } as CSSProperties;
        return (
          <div
            key={leaf.id}
