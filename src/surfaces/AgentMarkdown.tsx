      ...
      if (node.type === "element" && node.tagName === "img") {
        ...
        if (
          ...
          try {
            ...
            /* Streaming URL may still be incomplete. */
          }
          const path = resolveWorkspacePath(decoded, cwd);
          if (path)
            node.properties.src = encodeURI(path).replace(
              /[?#<>]/g,
...
function localFileLinks({ cwd }: { cwd?: string }) {
  return (tree: Root) => {
    const visit = (node: Root | RootContent) => {
      if (
        node.type === "element" &&
...
function MarkdownLink({
  ...
  const file =
    !allowRemoteMedia && href ? resolveFileLink(href, cwd) : undefined;
    ...
    return (
      <code
        ...
        className={`inline-flex items-center gap-1 rounded-md bg-content/8 px-1.5 min-h-6 max-w-full [overflow-wrap:anywhere] align-baseline font-mono text-[0.8em] text-content ${
          open ? "chat-reference cursor-pointer" : ""
        } ${className ?? ""}`}
...
function MarkdownImage({
  ...
  if (!allowRemoteMedia) {
    ...
    try {
      ...
      /* Keep incomplete streaming URLs intact. */
    }
    const path = resolveWorkspacePath(decoded, cwd);
    if (path)
      return (
  ...
  return <InboxMedia src={url} alt={alt} cwd={cwd} />;
}
