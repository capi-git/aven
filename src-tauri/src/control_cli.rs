...
const USAGE: &str = r#"Aven local control — supervise this orchestration run from the lead agent.
...
pub fn run(args: Vec<String>) -> i32 {
    ...
    match send(&action, &input, &request_id) {
        Ok(mut value) => {
            ...
            // Aven may have timed out waiting on its own executor, so a
...
fn loopback_address(endpoint: &str) -> Result<SocketAddr, String> {
    let address: SocketAddr = endpoint.parse().map_err(|_| "Invalid Aven endpoint")?;
    if !address.ip().is_loopback() || address.port() == 0 {
        return Err("Aven control requires a loopback endpoint with a nonzero port".into());
    }
    Ok(address)
}
...
fn read_reply(source: impl Read) -> Result<Value, Failure> {
    let mut line = String::new();
    BufReader::new(source)
        .take(2_000_001)
        .read_line(&mut line)
        .map_err(|e| sent(format!("No reply from Aven: {e}")))?;
    if line.len() > 2_000_000 {
        return Err(sent("Aven response is too large"));
    }
    serde_json::from_str(&line).map_err(|_| sent("Aven returned an invalid response"))
}
...
mod tests {
    ...
    fn control_endpoint_rejects_remote_wildcard_and_hostname_addresses() {
        for endpoint in ["127.0.0.1:1234", "127.1.2.3:1234", "[::1]:1234"] {
            assert!(loopback_address(endpoint).is_ok());
        }
        for endpoint in [
            "0.0.0.0:1234",
            "192.168.1.1:1234",
            ...
            assert!(loopback_address(endpoint).is_err());
        }
    }

    #[test]
    fn disconnected_control_replies_return_retry_aware_errors() {
        struct Reset;
        impl Read for Reset {
        ...
        let reset = read_reply(Reset).expect_err("reset must be an error");
        assert!(reset.sent);
        assert!(reset.error.starts_with("No reply from Aven:"));
        let closed = read_reply(&b""[..]).expect_err("EOF must be an error");
        assert!(closed.sent);
        assert_eq!(closed.error, "Aven returned an invalid response");
        assert_eq!(
