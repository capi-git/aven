...
#[tauri::command]
pub async fn fetch_inbox_media(
    url: String,
    cwd: Option<String>,
) -> Result<tauri::ipc::Response, String> {
    let bytes =
        tauri::async_runtime::spawn_blocking(move || fetch_inbox_media_sync(&url, cwd.as_deref()))
            .await
            .map_err(|error| error.to_string())??;
    Ok(tauri::ipc::Response::new(bytes))
}

fn fetch_inbox_media_sync(url: &str, cwd: Option<&str>) -> Result<Vec<u8>, String> {
    let mut current = parse_allowed_media_url(url)?;
    let token = if github_auth_host(&current.host) {
        github_auth_token(cwd)?
    } else {
        None
    };
...
fn github_auth_token(cwd: Option<&str>) -> Result<Option<String>, String> {
    // Public media without a repository context stays anonymous. Never borrow
    // whichever account is currently active in the shared CLI.
...
mod tests {
    ...
    #[test]
    fn contextless_media_never_borrows_an_active_github_account() {
        assert_eq!(github_auth_token(None).unwrap(), None);
        assert_eq!(github_auth_token(Some(" ")).unwrap(), None);
        assert!(github_auth_token(Some("/nonexistent-covecode-repository")).is_err());
    }
