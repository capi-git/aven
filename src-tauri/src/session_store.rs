...
fn ensure_orchestration_history(conn: &Connection) -> rusqlite::Result<()> {
    let indexed: bool = conn.query_row(
        "SELECT COUNT(*) = 2 FROM sqlite_master WHERE type = 'table' AND name IN ('orchestration_sidebar', 'orchestration_workers')",
        [],
        |row| row.get(0),
    )?;
    ...
    conn.query_row(
        ...
        |row| {
            ...
            Ok(SessionRecord {
                ...
                blocks,
                queued_messages: serde_json::from_str(&row.get::<_, String>(15)?).map_err(|e| {
                    rusqlite::Error::FromSqlConversionFailure(
                        15,
                        rusqlite::types::Type::Text,
                        Box::new(e),
...
mod tests {
    ...
    #[test]
    fn queued_followups_survive_migration_and_can_be_cleared() {
        let store = SessionStore::open_in_memory().unwrap();
        let conn = store.lock_conn().unwrap();
        let mut session = sample("queued", "/tmp/project", "Queue");
        upsert_session(&conn, &session).unwrap();
        // Simulate an existing latest-version database without the new column.
        conn.execute("ALTER TABLE sessions DROP COLUMN queued_messages_json", [])
            .unwrap();
        migrate(&conn).unwrap();
        assert!(get_session(&conn, "queued")
            .unwrap()
            .unwrap()
            .queued_messages
            .is_empty());
        session.queued_messages = vec![
            json!({"id":"next", "text":"Follow up", "attachments":[{"id":"png", "path":"/attachments/screen.png"}], "intent":"plan"}),
        ];
        upsert_session(&conn, &session).unwrap();
        migrate(&conn).unwrap();
        let restored = get_session(&conn, "queued").unwrap().unwrap();
        ...
        assert!(get_session(&conn, "queued")
            ...
            .is_empty());
        assert_eq!(
            get_session(&conn, "queued").unwrap().unwrap().blocks,
            session.blocks
        );
    }
    ...
    #[test]
    fn worker_ownership_and_transcript_are_saved_atomically() {
        let store = SessionStore::open_in_memory().unwrap();
        let conn = store.conn.lock().unwrap();
        let mut worker = sample("worker", "/tmp/a", "Worker");
        worker.blocks = json!([{"role":"user","text":"Task","orchestrationLeadId":"lead"}]);
        worker.queued_messages = vec![json!({"text":"Keep this follow-up"})];
        conn.execute_batch("CREATE TRIGGER reject_worker BEFORE INSERT ON orchestration_workers BEGIN SELECT RAISE(ABORT, 'test failure'); END;").unwrap();
        assert!(upsert_session(&conn, &worker).is_err());
        assert!(get_session(&conn, "worker").unwrap().is_none());
        conn.execute_batch("DROP TRIGGER reject_worker;").unwrap();
        upsert_session(&conn, &worker).unwrap();
        let stored = get_session(&conn, "worker").unwrap().unwrap();
        assert_eq!(stored.orchestration_lead_id.as_deref(), Some("lead"));
        assert_eq!(stored.queued_messages, worker.queued_messages);
    }

    #[test]
    fn migration_repairs_a_missing_worker_index_without_rewriting_the_run() {
        let store = SessionStore::open_in_memory().unwrap();
        let conn = store.conn.lock().unwrap();
        upsert_session(&conn, &sample("worker", "/tmp/a", "Worker")).unwrap();
        let run = json!({"status":"active","tasks":[{"sessionId":"worker","title":"Task"}]});
        save_orchestration(&conn, "lead", &run).unwrap();
        conn.execute_batch("DROP TABLE orchestration_workers;")
            .unwrap();
        migrate(&conn).unwrap();
        assert_eq!(
        ...
        let raw: String = conn
            ...
            .unwrap();
        assert_eq!(serde_json::from_str::<Value>(&raw).unwrap(), run);
    }
