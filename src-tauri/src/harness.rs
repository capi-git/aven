...
struct HarnessInner {
    ...
    stopping: HashMap<String, Vec<Arc<LiveChild>>>,
}

pub struct HarnessHost {
    inner: Mutex<HarnessInner>,
    sse: Mutex<HashMap<String, Arc<LiveSse>>>,
    session_gates: Mutex<HashMap<String, Weak<Mutex<()>>>>,
...
impl HarnessHost {
    ...
    // Spawn and confirmed stop for one session cannot overtake each other.
    // Weak entries are pruned; unrelated sessions keep independent gates.
    fn session_gate(&self, session_id: &str) -> Arc<Mutex<()>> {
        let mut gates = self.session_gates.lock().unwrap_or_else(|e| e.into_inner());
    ...
    fn stop_session(&self, session_id: &str) -> Result<(), String> {
        ...
        self.stop_session_locked(session_id)
    }

    fn stop_session_locked(&self, session_id: &str) -> Result<(), String> {
        self.stop_sse(session_id);
        // Keep the captured generation reachable until death is confirmed, so
        // concurrent/retried stops cannot report success by finding no child.
        let children = {
            let mut inner = self.lock_inner();
            *inner.epochs.entry(session_id.to_string()).or_insert(0) += 1;
            if let Some(live) = inner.children.remove(session_id) {
                inner
                    .stopping
        ...
        for child in children {
            ...
            if let Some(stopping) = inner.stopping.get_mut(session_id) {
                stopping.retain(|entry| entry.pid != child.pid);
                if stopping.is_empty() {
                    inner.stopping.remove(session_id);
                }
            }
        }
        Ok(())
    ...
    fn remove_if_pid(&self, session_id: &str, pid: u32) -> Option<Arc<LiveChild>> {
        ...
        let live = inner.children.remove(session_id);
        #[cfg(not(windows))]
        if let Some(child) = &live {
            // The leader can exit before TERM-resistant descendants do.
            if tree_alive(child.pid) {
                inner
                    .stopping
                    .entry(session_id.to_string())
                    .or_default()
    ...
    pub(crate) fn kill_all(&self) {
        let kids: Vec<Arc<LiveChild>> = {
            ...
            let mut children: Vec<_> = inner.children.drain().map(|(_, child)| child).collect();
            children.extend(inner.stopping.drain().flat_map(|(_, children)| children));
            children
        };
        self.stop_all_sse();
...
pub fn harness_spawn(
    ...
    let _guard = gate.lock().unwrap_or_else(|e| e.into_inner());
    host.stop_session_locked(&session_id)?;
    let (epoch, kill_all, _) = host.begin_spawn(&session_id);
    ...
    configure_browser_environment(
        &mut cmd,
        crate::browser_agent::ensure_child_environment(&caller, &session_id),
    );
    ...
    if let Some(rejected) = host.install_spawn(session_id.clone(), epoch, kill_all, live) {
        ...
        terminate_confirmed(rejected.pid)?;
...
/// access and never falls back to credentials inherited from another process.
fn configure_browser_environment(
    ...
    match environment {
        Ok(values) => { cmd.envs(values); }
        Err(_) => eprintln!("[browser-agent] Browser connection unavailable; starting agent without browser access."),
    }
}
...
pub fn harness_sse_open(
    ...
    thread::spawn(move || {
        ...
        match result {
            Ok(response) => {
                if let Err(error) = reject_harness_redirect(response.status()) {
...
fn read_http_response(response: ureq::Response) -> Result<HarnessHttpResponse, String> {
    ...
    reject_harness_redirect(status)?;
    let body = response
        .into_string()
...
fn assert_loopback(url: &str) -> Result<(), String> {
    let parsed = tauri::Url::parse(url).map_err(|_| "Invalid OpenCode HTTP URL")?;
...
fn harness_http_agent() -> ureq::AgentBuilder {
    // Redirects could move prompts or event streams off the approved server.
    // Disable proxy discovery too: these requests must remain on this Mac.
    ureq::AgentBuilder::new()
        .redirects(0)
        .try_proxy_from_env(false)
}

fn reject_harness_redirect(status: u16) -> Result<(), String> {
    if (300..400).contains(&status) {
        Err("OpenCode HTTP redirects are not allowed".into())
    } else {
        Ok(())
    }
}
...
/// Run off the UI thread and fail closed if tree death cannot be confirmed.
fn terminate_confirmed(pid: u32) -> Result<(), String> {
    if pid <= 1 {
        return Err("Refusing to terminate an invalid harness pid".into());
    }
    #[cfg(not(windows))]
    {
        signal_tree(pid, TreeSignal::Term);
        wait_until_dead(&[pid], Instant::now() + KILL_ESCALATE);
        if tree_alive(pid) {
            signal_tree(pid, TreeSignal::Kill);
            wait_until_dead(&[pid], Instant::now() + Duration::from_secs(2));
        }
        if tree_alive(pid) {
            return Err(format!(
                "Harness process group {pid} is still running; cancellation is incomplete"
            ));
        }
    }
    #[cfg(windows)]
    {
        // taskkill /T /F is synchronous; preserve failures for the caller.
        ...
        let status = cmd
            ...
            .stderr(Stdio::null())
            .status()
            .map_err(|e| format!("Failed to stop harness {pid}: {e}"))?;
        if !status.success() {
            return Err(format!("Harness {pid} termination could not be confirmed"));
        }
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn harness_http_rejects_credentials_and_lookalike_loopback_hosts() {
        for url in [
        ...
        for url in [
            "http://127.0.0.1:4096@outside.invalid/",
            "http://localhost:80@outside.invalid/",
            "http://user@127.0.0.1:4096/",
            "http://127.0.0.1.evil:4096/",
            "http://localhost.evil:4096/",
            "https://127.0.0.1:4096/",
            "file:///tmp/session",
            "http://127.0.0.1:0/",
            "http://127.0.0.1:4096/#elsewhere",
        ] {
    ...
    #[test]
    fn harness_http_does_not_follow_redirects() {
        ...
        let server = thread::spawn(move || {
            ...
            socket.write_all(b"HTTP/1.1 307 Temporary Redirect\r\nLocation: http://outside.invalid/session\r\nContent-Length: 0\r\nConnection: close\r\n\r\n").unwrap();
        });
        let response = harness_http_agent()
        ...
        assert!(read_http_response(response)
            .err()
            .unwrap()
            .contains("redirects"));
        server.join().unwrap();
        for status in [301, 302, 303, 307, 308] {
            assert!(reject_harness_redirect(status).is_err());
        }
        assert!(reject_harness_redirect(200).is_ok());
    }
    ...
    #[test]
    fn session_stop_waits_for_resistant_descendants_and_concurrent_stops() {
        let host = Arc::new(HarnessHost::new());
        let (epoch, generation, _) = host.begin_spawn("stop-test");
        let (live, mut child) = live_group(
            r#"trap '' TERM; sh -c 'trap "" TERM; while true; do sleep 1; done' & wait"#,
        );
        let pid = live.pid;
        host.install_spawn("stop-test".into(), epoch, generation, live);
        ...
        let stop = thread::spawn(move || {
            peer.stop_session("stop-test").expect("confirmed stop");
            assert!(!tree_alive(pid), "stop resolved before tree exit");
        });
        host.stop_session("stop-test")
            .expect("concurrent confirmed stop");
        assert!(!tree_alive(pid), "concurrent stop released ownership early");
        stop.join().unwrap();
        waiter.join().unwrap();
    }

    #[test]
    fn session_stop_does_not_terminate_an_unrelated_session() {
        let host = HarnessHost::new();
        let (epoch, generation, _) = host.begin_spawn("keep");
        let (live, child) = live_group("sleep 30");
        let pid = live.pid;
        host.install_spawn("keep".into(), epoch, generation, live);
        host.stop_session("missing").unwrap();
        assert!(tree_alive(pid));
        let waiter = thread::spawn(move || {
            let mut child = child;
            let _ = child.wait();
        });
        host.stop_session("keep").unwrap();
        waiter.join().unwrap();
    }
